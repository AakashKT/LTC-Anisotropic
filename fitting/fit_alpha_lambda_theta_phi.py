...
def optimize_loop(args, model, target):
        ...
        for lambd in range(8):
            for theta in range(8):
                for phi in range(8):
                    ...
                    # store
                    alphax_list[alpha, lambd, theta, phi] = ax
                    alphay_list[alpha, lambd, theta, phi] = ay
                    theta_list[alpha, lambd, theta, phi] = t
                    phi_list[alpha, lambd, theta, phi] = p
    ...
    np.save('%s/alpha_lambda_theta_phi.npy' % (args.savedir), ltc_mat)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_bins', type=int, default=8)
    ...
    lut_size = (args.alpha_bins, args.lambda_bins, args.theta_bins, args.phi_bins)
